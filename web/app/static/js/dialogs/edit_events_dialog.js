...
export class EditEventsDialog extends BaseDialog {
    constructor(evented_day, on_save = () => {}) {
        super()
        this.dialog = document.getElementById("edit_events_dialog")
        this.dialog_name = "Edit Events Dialog"
        this.evented_day = evented_day
        this.edit_events_dialog_save_button = this.dialog.querySelector(".button-save")
        this.edit_events_dialog_cancel_button = this.dialog.querySelector(".button-cancel")
        this.added_events_types_list = document.getElementById("edit_events_dialog_added_events_types_list")
        this.events_types_list = document.getElementById("edit_events_dialog_events_types_list")
        ...
        this.edit_events_dialog_cancel_button.onclick = () => {
            this.hide()
        }
        this.edit_events_dialog_save_button.onclick = () => {

            manageEvents(getLocalDate(evented_day.date), this.added_events_types.keys().toArray(), () => {
                this.hide()
                this.on_save()
    ...
    load_events_on_day() {
        ...
        for (let i = 0; i < this.evented_day.events_types.length; i++) {
            // if (i % this.dialog_events_cards_count == 0 && i != 0) {
            ...
            let events_type = this.evented_day.events_types[i]
            let events_type_element = this.events_types_list.querySelector(`li[data-id="${events_type.id}"]`)
